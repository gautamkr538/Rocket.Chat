...
- **Retrieve Messages** – Fetch message history from channels or direct rooms  
- **Send Attachments** – Upload and share attachments with messages  

### 4️⃣ WebSocket Simulation
- Simulates receiving a **message payload** for testing  
...
| **Service Layer** | Business logic for Admin, User, and Messaging operations |
...
If running Rocket.Chat locally with Docker Compose:

bash
Copy
Edit
docker-compose up -d
Ensure PostgreSQL is running inside the Rocket.Chat container.
...
/chat/send-attachment	POST	Send message with attachments
...
Extend support for advanced attachment handling
